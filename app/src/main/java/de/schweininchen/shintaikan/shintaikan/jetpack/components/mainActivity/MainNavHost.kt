...
fun MainNavHost(
    ...
    NavHost(
        ...
        composable(Destinations.HOME.id, deepLinks = listOf(navDeepLink { uriPattern = "shintaikan.de" })) {
            Home(
                wordpressList,
                firestoreDataNotEmpty = viewModel.firestoreData.isNotEmpty(),
                trplanData = viewModel.trplanData,
                isConnected = viewModel.isConnected
            )
        }
        composable(Destinations.TRPLAN.id, deepLinks = listOf(navDeepLink { uriPattern = "shintaikan.de/\$page_id=18" })) {
        ...
        /*composable(
            ...
            Text(text = backStackEntry.arguments?.getString("id").toString())
